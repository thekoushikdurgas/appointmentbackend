#!/usr/bin/env python3
"""Dedicated test runner for usage API endpoints."""
...
from fixtures.usage_test_scenarios import UsageTestScenarios


def main():
    """Main entry point for usage API testing."""
    parser = argparse.ArgumentParser(
        description="Test Usage API endpoints comprehensively",
...
Examples:
  ...
  python test_usage_api.py --category track_usage
  
  # Custom output directory
  python test_usage_api.py --output-dir ./usage_test_reports
  
  # Comprehensive mode
  python test_usage_api.py --mode comprehensive
        """
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Output directory for reports (default: ./test_reports/usage_api)"
    )
    parser.add_argument(
        "--category",
        type=str,
        choices=["current_usage", "track_usage", "reset_usage"],
        default=None,
    ...
    console.print(Panel.fit(
        "[bold cyan]Usage API Test Suite[/bold cyan]\n"
    ...
    if args.output_dir:
        ...
        output_dir = Path(__file__).parent / "test_reports" / "usage_api"
    ...
    scenarios = UsageTestScenarios.get_all_scenarios()
    if args.category:
    ...
    for category, category_scenarios in scenarios.items():
        for scenario in category_scenarios:
            ...
            endpoint = {
                ...
                "endpoint": scenario.get("endpoint", "/api/v1/usage/current/"),
                "api_version": "v1" if "/api/v1/" in scenario.get("endpoint", "") else "v2",
                "category": category.replace("_", " ").title(),
                "description": scenario.get("description", ""),
                "requires_auth": True,
                "requires_admin": False  # Usage endpoints don't require admin
            }
    ...
    # Execute tests
    console.print("\n[bold]Testing usage API endpoints...[/bold]")
    ...
    # Display summary
    console.print("\n")
    summary_table = Table(title="Usage API Test Results Summary", show_header=True, header_style="bold cyan")
    ...
    # Display feature breakdown for track/reset tests
    feature_stats = {}
    for result in all_results.get("detailed_results", []):
        if result.get("endpoint") in ["/api/v1/usage/track/", "/api/v2/usage/reset"]:
            # Try to extract feature name from request body
            request_body = result.get("request", {}).get("body", {})
            if isinstance(request_body, dict):
                feature = request_body.get("feature", "unknown")
    ...
    if feature_stats:
        console.print("\n")
        feature_table = Table(title="Results by Feature", show_header=True, header_style="bold cyan")
        feature_table.add_column("Feature", style="cyan")
        feature_table.add_column("Total", justify="right")
        feature_table.add_column("Passed", justify="right", style="green")
        feature_table.add_column("Failed", justify="right", style="red")
        feature_table.add_column("Success Rate", justify="right")
