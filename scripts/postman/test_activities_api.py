#!/usr/bin/env python3
"""Dedicated test runner for Activities API endpoints."""
...
from fixtures.activities_test_scenarios import ActivitiesTestScenarios


def main():
    """Main entry point for Activities API testing."""
    parser = argparse.ArgumentParser(
        description="Test Activities API endpoints comprehensively",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Basic test run
  python test_activities_api.py
  
  # Test specific category
  python test_activities_api.py --category list
  
  # Custom output directory
  python test_activities_api.py --output-dir ./activities_test_reports
  
  # Comprehensive mode
  python test_activities_api.py --mode comprehensive
  
  # Note: All Activities endpoints require authentication (all roles can access)
        """
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Test specific category (list, stats, list_errors, stats_errors)"
    )
    parser.add_argument(
        "--output-dir",
        type=str,
        default="./activities_test_reports",
        help="Output directory for test reports (default: ./activities_test_reports)"
    )
    parser.add_argument(
        "--mode",
        type=str,
    ...
    console.print(Panel.fit(
        "[bold cyan]Activities API Test Runner[/bold cyan]\n"
        "Comprehensive testing for all Activities API endpoints",
        border_style="cyan"
    ))
    ...
    # Get all scenarios
    all_scenarios = ActivitiesTestScenarios.get_all_scenarios()
    ...
    report_path = reporter.generate_report(
        ...
        test_name="Activities API Tests"
    )
