#!/usr/bin/env python3
"""Dedicated test runner for Billing API endpoints."""
...
from fixtures.billing_test_scenarios import BillingTestScenarios


def main():
    """Main entry point for Billing API testing."""
    parser = argparse.ArgumentParser(
        description="Test Billing API endpoints comprehensively",
...
Examples:
  ...
  python test_billing_api.py --category subscribe
  
  # Custom output directory
  python test_billing_api.py --output-dir ./billing_test_reports
  
  # Comprehensive mode
  python test_billing_api.py --mode comprehensive
  
  # Note: Most endpoints require authentication. Admin endpoints require SuperAdmin role.
        """
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Test specific category (billing_info, plans, addons, subscribe, addon_purchase, cancel, invoices, admin_plans, admin_addons, or their _errors variants)"
    )
    parser.add_argument(
        "--output-dir",
        type=str,
        default="./billing_test_reports",
        help="Output directory for test reports (default: ./billing_test_reports)"
    )
    parser.add_argument(
        "--mode",
        type=str,
    ...
    console.print(Panel.fit(
        "[bold cyan]Billing API Test Runner[/bold cyan]\n"
        "Comprehensive testing for all Billing API endpoints",
        border_style="cyan"
    ))
    ...
    # Get all scenarios
    all_scenarios = BillingTestScenarios.get_all_scenarios()
    ...
    # Filter by mode
    if args.mode == "smoke":
        all_scenarios = [s for s in all_scenarios if "errors" not in s.get("category", "") and "admin" not in s.get("category", "")]
    ...
    with Progress(
        ...
        for scenario in all_scenarios:
            ...
            endpoint = {
                ...
                "api_version": scenario.get("endpoint", "").split("/")[2] if len(scenario.get("endpoint", "").split("/")) > 2 else "v1",
            ...
            test_case = {
                ...
                "requires_admin": scenario.get("requires_admin", False)
            }
