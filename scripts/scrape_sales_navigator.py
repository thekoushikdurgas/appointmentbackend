#!/usr/bin/env python3
"""
Scrape LinkedIn Sales Navigator HTML file and extract profile data to JSON.

This script parses a Sales Navigator search results HTML file and extracts
...
def create_output_structure(profiles: List[Dict], page_metadata: Dict, 
                           source_file: str, output_format: str = 'hierarchical') -> Dict:
    """
...
def scrape_sales_navigator_html(html_file_path: str, include_metadata: bool = True) -> Tuple[List[Dict], Dict]:
    """
    Scrape Sales Navigator HTML file and extract all profile data.
    
    Args:
        html_file_path: Path to the HTML file to scrape
    ...
    print(f"Found {len(profile_elements)} profile(s)")
    ...
    for idx, profile_elem in enumerate(profile_elements, 1):
        print(f"Extracting profile {idx}/{len(profile_elements)}...")
        try:
            profile_data = extract_profile_data(profile_elem)
            profiles.append(profile_data)
        except Exception as e:
            print(f"  Warning: Error extracting profile {idx}: {e}", file=sys.stderr)
            continue
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Scrape LinkedIn Sales Navigator HTML and extract profile data'
    )
    parser.add_argument(
        'input_file',
        nargs='?',
        help='Path to HTML file (default: scripts/html/salesivigator1.html)'
    )
    parser.add_argument(
        '-o', '--output',
        help='Output JSON file path (default: scripts/sales_navigator_profiles.json)'
    )
    parser.add_argument(
        '--output-format',
        choices=['flat', 'hierarchical'],
        default='hierarchical',
        help='Output format: flat (list) or hierarchical (default: hierarchical)'
    )
    parser.add_argument(
        '--include-metadata',
        action='store_true',
        default=True,
        help='Include page-level metadata (default: True)'
    )
    parser.add_argument(
        '--no-metadata',
        dest='include_metadata',
        action='store_false',
        help='Exclude page-level metadata'
    )
    parser.add_argument(
        '--min-quality-score',
        type=int,
        default=0,
        help='Minimum data quality score to include profile (0-100, default: 0)'
    )
    
    args = parser.parse_args()
    
    # Default input file path
    default_input = Path(__file__).parent / 'html' / 'salesivigator1.html'
    input_file = args.input_file if args.input_file else str(default_input)
    
    # Default output file path
    default_output = Path(__file__).parent / 'sales_navigator_profiles.json'
    output_file = args.output if args.output else str(default_output)
    
    try:
        # Scrape the HTML file
        profiles, page_metadata = scrape_sales_navigator_html(input_file, args.include_metadata)
        
        # Filter out profiles with no name (likely invalid)
        valid_profiles = [p for p in profiles if p.get('name')]
        
        # Filter by quality score if specified
        if args.min_quality_score > 0:
            # Calculate quality for filtering
            for profile in valid_profiles:
                if 'data_quality_score' not in profile:
                    quality = calculate_data_quality(profile)
                    profile['data_quality_score'] = quality['data_quality_score']
            valid_profiles = [
                p for p in valid_profiles
                if p.get('data_quality_score', 0) >= args.min_quality_score
            ]
        
        print(f"\nExtracted {len(valid_profiles)} valid profile(s)")
        ...
        print(f"Successfully saved {len(valid_profiles)} profiles to {output_path}")
        
        # Print summary
        print("\nSummary:")
        print(f"  Total profiles extracted: {len(valid_profiles)}")
        profiles_with_company = sum(1 for p in valid_profiles if p.get('company'))
        profiles_with_title = sum(1 for p in valid_profiles if p.get('title'))
        profiles_with_location = sum(1 for p in valid_profiles if p.get('location'))
        profiles_with_lead_id = sum(1 for p in valid_profiles if p.get('lead_id'))
        profiles_premium = sum(1 for p in valid_profiles if p.get('is_premium_member'))
        print(f"  Profiles with company: {profiles_with_company}")
        print(f"  Profiles with title: {profiles_with_title}")
        print(f"  Profiles with location: {profiles_with_location}")
        print(f"  Profiles with lead_id: {profiles_with_lead_id}")
        print(f"  Premium members: {profiles_premium}")
        
        if args.output_format == 'hierarchical' and isinstance(output_data, dict):
            avg_quality = sum(p.get('data_quality_score', 0) for p in valid_profiles) / len(valid_profiles) if valid_profiles else 0
