-- ============================================================================
-- Endpoint: GET /api/v1/contacts/keywords/
-- API Version: v1
-- Description: Return distinct values for /contacts/keywords/ using AttributeListParams. The separated parameter controls whether array columns are split into unique tokens.
-- ============================================================================
--
-- Parameters:
--   Query Parameters:
...
-- GET /api/v1/contacts/keywords/?separated=false
SELECT DISTINCT array_to_string(co.keywords, ',') as value
FROM contacts c
LEFT JOIN companies co ON c.company_id = co.uuid
...
-- GET /api/v1/contacts/keywords/?distinct=true&separated=true
SELECT DISTINCT unnest(co.keywords) as value
FROM contacts c
LEFT JOIN companies co ON c.company_id = co.uuid
LEFT JOIN contacts_metadata cm ON c.uuid = cm.uuid
...
-- GET /api/v1/contacts/keywords/?distinct=false&separated=true
SELECT unnest(co.keywords) as value
FROM contacts c
LEFT JOIN companies co ON c.company_id = co.uuid
LEFT JOIN contacts_metadata cm ON c.uuid = cm.uuid
LEFT JOIN companies_metadata com ON co.uuid = com.uuid
...
-- GET /api/v1/contacts/keywords/?ordering=value&separated=true
SELECT DISTINCT unnest(co.keywords) as value
FROM contacts c
LEFT JOIN companies co ON c.company_id = co.uuid
LEFT JOIN contacts_metadata cm ON c.uuid = cm.uuid
...
WHERE co.keywords IS NOT NULL
    AND array_length(co.keywords, 1) > 0
    AND EXISTS (
        SELECT 1 FROM unnest(co.keywords) AS keyword 
        WHERE keyword ILIKE '%cyber%'
    )
ORDER BY value ASC
LIMIT 25
OFFSET 0;
...
-- GET /api/v1/contacts/keywords/?search=cyber&separated=false
SELECT DISTINCT array_to_string(co.keywords, ',') as value
FROM contacts c
LEFT JOIN companies co ON c.company_id = co.uuid
LEFT JOIN contacts_metadata cm ON c.uuid = cm.uuid
...
WHERE co.keywords IS NOT NULL
    AND array_length(co.keywords, 1) > 0
    AND array_to_string(co.keywords, ',') ILIKE '%cyber%'
ORDER BY array_to_string(co.keywords, ',') ASC
LIMIT 25
OFFSET 0;

-- Query 10: With company filter and separated=true
-- GET /api/v1/contacts/keywords/?company=Bandura&separated=true
SELECT DISTINCT unnest(co.keywords) as value
FROM contacts c
LEFT JOIN companies co ON c.company_id = co.uuid
LEFT JOIN contacts_metadata cm ON c.uuid = cm.uuid
...
-- GET /api/v1/contacts/keywords/?limit=50&separated=true
SELECT DISTINCT unnest(co.keywords) as value
FROM contacts c
LEFT JOIN companies co ON c.company_id = co.uuid
LEFT JOIN contacts_metadata cm ON c.uuid = cm.uuid
...
-- GET /api/v1/contacts/keywords/?offset=25&separated=true
SELECT DISTINCT unnest(co.keywords) as value
FROM contacts c
LEFT JOIN companies co ON c.company_id = co.uuid
LEFT JOIN contacts_metadata cm ON c.uuid = cm.uuid
...
-- GET /api/v1/contacts/keywords/?limit=10&offset=20&separated=true
SELECT DISTINCT unnest(co.keywords) as value
FROM contacts c
LEFT JOIN companies co ON c.company_id = co.uuid
LEFT JOIN contacts_metadata cm ON c.uuid = cm.uuid
...
-- GET /api/v1/contacts/keywords/?search=cyber&company=Bandura&separated=true
SELECT DISTINCT unnest(co.keywords) as value
FROM contacts c
LEFT JOIN companies co ON c.company_id = co.uuid
LEFT JOIN contacts_metadata cm ON c.uuid = cm.uuid
...
-- GET /api/v1/contacts/keywords/?distinct=true&ordering=-value&search=cyber&separated=true
SELECT DISTINCT unnest(co.keywords) as value
FROM contacts c
LEFT JOIN companies co ON c.company_id = co.uuid
LEFT JOIN contacts_metadata cm ON c.uuid = cm.uuid
...
-- GET /api/v1/contacts/keywords/?distinct=true&limit=50&offset=0&ordering=value&search=cyber&company=Bandura&separated=true
SELECT DISTINCT unnest(co.keywords) as value
FROM contacts c
LEFT JOIN companies co ON c.company_id = co.uuid
LEFT JOIN contacts_metadata cm ON c.uuid = cm.uuid
...
-- GET /api/v1/contacts/keywords/?first_name=Patrick&separated=true
SELECT DISTINCT unnest(co.keywords) as value
FROM contacts c
LEFT JOIN companies co ON c.company_id = co.uuid
LEFT JOIN contacts_metadata cm ON c.uuid = cm.uuid
...
-- GET /api/v1/contacts/keywords/?last_name=McGarry&separated=true
SELECT DISTINCT unnest(co.keywords) as value
FROM contacts c
LEFT JOIN companies co ON c.company_id = co.uuid
LEFT JOIN contacts_metadata cm ON c.uuid = cm.uuid
...
-- GET /api/v1/contacts/keywords/?seniority=C suite&separated=true
SELECT DISTINCT unnest(co.keywords) as value
FROM contacts c
LEFT JOIN companies co ON c.company_id = co.uuid
LEFT JOIN contacts_metadata cm ON c.uuid = cm.uuid
...
-- GET /api/v1/contacts/keywords/?email=banduracyber.com&separated=true
SELECT DISTINCT unnest(co.keywords) as value
FROM contacts c
LEFT JOIN companies co ON c.company_id = co.uuid
LEFT JOIN contacts_metadata cm ON c.uuid = cm.uuid
...
-- GET /api/v1/contacts/keywords/?department=C-Suite&separated=true
SELECT DISTINCT unnest(co.keywords) as value
FROM contacts c
LEFT JOIN companies co ON c.company_id = co.uuid
LEFT JOIN contacts_metadata cm ON c.uuid = cm.uuid
...
-- GET /api/v1/contacts/keywords/?employees_min=20&separated=true
SELECT DISTINCT unnest(co.keywords) as value
FROM contacts c
LEFT JOIN companies co ON c.company_id = co.uuid
LEFT JOIN contacts_metadata cm ON c.uuid = cm.uuid
...
-- GET /api/v1/contacts/keywords/?employees_max=100&separated=true
SELECT DISTINCT unnest(co.keywords) as value
FROM contacts c
LEFT JOIN companies co ON c.company_id = co.uuid
LEFT JOIN contacts_metadata cm ON c.uuid = cm.uuid
...
-- GET /api/v1/contacts/keywords/?employees_min=20&employees_max=100&separated=true
SELECT DISTINCT unnest(co.keywords) as value
FROM contacts c
LEFT JOIN companies co ON c.company_id = co.uuid
LEFT JOIN contacts_metadata cm ON c.uuid = cm.uuid
...
-- GET /api/v1/contacts/keywords/?industries=information technology&separated=true
SELECT DISTINCT unnest(co.keywords) as value
FROM contacts c
LEFT JOIN companies co ON c.company_id = co.uuid
LEFT JOIN contacts_metadata cm ON c.uuid = cm.uuid
...
-- Query 26: With ContactFilterParams - technologies filter and separated=true
-- GET /api/v1/contacts/keywords/?technologies=Salesforce&separated=true
SELECT DISTINCT unnest(co.keywords) as value
FROM contacts c
LEFT JOIN companies co ON c.company_id = co.uuid
LEFT JOIN contacts_metadata cm ON c.uuid = cm.uuid
...
-- GET /api/v1/contacts/keywords/?city=Miamisburg&separated=true
SELECT DISTINCT unnest(co.keywords) as value
FROM contacts c
LEFT JOIN companies co ON c.company_id = co.uuid
LEFT JOIN contacts_metadata cm ON c.uuid = cm.uuid
...
-- GET /api/v1/contacts/keywords/?state=Ohio&separated=true
SELECT DISTINCT unnest(co.keywords) as value
FROM contacts c
LEFT JOIN companies co ON c.company_id = co.uuid
LEFT JOIN contacts_metadata cm ON c.uuid = cm.uuid
...
-- GET /api/v1/contacts/keywords/?country=United States&separated=true
SELECT DISTINCT unnest(co.keywords) as value
FROM contacts c
LEFT JOIN companies co ON c.company_id = co.uuid
LEFT JOIN contacts_metadata cm ON c.uuid = cm.uuid
...
-- GET /api/v1/contacts/keywords/?annual_revenue_min=7000000&separated=true
SELECT DISTINCT unnest(co.keywords) as value
FROM contacts c
LEFT JOIN companies co ON c.company_id = co.uuid
LEFT JOIN contacts_metadata cm ON c.uuid = cm.uuid
...
-- GET /api/v1/contacts/keywords/?annual_revenue_max=9000000&separated=true
SELECT DISTINCT unnest(co.keywords) as value
FROM contacts c
LEFT JOIN companies co ON c.company_id = co.uuid
LEFT JOIN contacts_metadata cm ON c.uuid = cm.uuid
...
-- Query 32: With ContactFilterParams - exclude_keywords filter and separated=true
-- GET /api/v1/contacts/keywords/?exclude_keywords=Legacy,Old&separated=true
SELECT DISTINCT unnest(co.keywords) as value
FROM contacts c
LEFT JOIN companies co ON c.company_id = co.uuid
LEFT JOIN contacts_metadata cm ON c.uuid = cm.uuid
...
WHERE co.keywords IS NOT NULL
    ...
    AND NOT EXISTS (
        ...
        WHERE array_to_string(co.keywords, ',') ILIKE '%' || exclude_val || '%'
    )
ORDER BY value ASC
LIMIT 25
OFFSET 0;
...
-- GET /api/v1/contacts/keywords/?exclude_company_ids=398cce44-233d-5f7c-aea1-e4a6a79df10c&separated=true
SELECT DISTINCT unnest(co.keywords) as value
FROM contacts c
LEFT JOIN companies co ON c.company_id = co.uuid
LEFT JOIN contacts_metadata cm ON c.uuid = cm.uuid
...
-- GET /api/v1/contacts/keywords/?created_at_after=2024-01-01T00:00:00&separated=true
SELECT DISTINCT unnest(co.keywords) as value
FROM contacts c
LEFT JOIN companies co ON c.company_id = co.uuid
LEFT JOIN contacts_metadata cm ON c.uuid = cm.uuid
...
-- GET /api/v1/contacts/keywords/?created_at_before=2024-12-31T23:59:59&separated=true
SELECT DISTINCT unnest(co.keywords) as value
FROM contacts c
LEFT JOIN companies co ON c.company_id = co.uuid
LEFT JOIN contacts_metadata cm ON c.uuid = cm.uuid
...
-- GET /api/v1/contacts/keywords/?company=Bandura&seniority=C suite&employees_min=20&employees_max=100&separated=true
SELECT DISTINCT unnest(co.keywords) as value
FROM contacts c
LEFT JOIN companies co ON c.company_id = co.uuid
LEFT JOIN contacts_metadata cm ON c.uuid = cm.uuid
...
-- GET /api/v1/contacts/keywords/?search=cyber&company=Bandura&distinct=true&ordering=-value&separated=true
SELECT DISTINCT unnest(co.keywords) as value
FROM contacts c
LEFT JOIN companies co ON c.company_id = co.uuid
LEFT JOIN contacts_metadata cm ON c.uuid = cm.uuid
...
-- GET /api/v1/contacts/keywords/?search=cyber&company=Bandura&separated=false
SELECT DISTINCT array_to_string(co.keywords, ',') as value
FROM contacts c
LEFT JOIN companies co ON c.company_id = co.uuid
LEFT JOIN contacts_metadata cm ON c.uuid = cm.uuid
...
-- GET /api/v1/contacts/keywords/?distinct=true&limit=50&offset=0&ordering=value&search=cyber&company=Bandura&seniority=C suite&employees_min=20&separated=true
SELECT DISTINCT unnest(co.keywords) as value
FROM contacts c
LEFT JOIN companies co ON c.company_id = co.uuid
LEFT JOIN contacts_metadata cm ON c.uuid = cm.uuid
