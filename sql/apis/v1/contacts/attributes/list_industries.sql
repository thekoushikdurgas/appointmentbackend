-- ============================================================================
-- Endpoint: GET /api/v1/contacts/industry/
-- API Version: v1
-- Description: Return distinct values for /contacts/industry/ using AttributeListParams. The separated parameter (true/false) controls whether array columns are split into unique tokens.
-- ============================================================================
--
-- Parameters:
--   Query Parameters:
...
--
-- Response Structure:
--   Returns array of strings: ["Information Technology", "Healthcare", ...] when separated=true
--   Returns array of comma-separated strings: ["Information Technology,Healthcare", ...] when separated=false
--
-- Response Codes:
--   200 OK: Industries retrieved successfully
--   400 Bad Request: Invalid query parameters
--   401 Unauthorized: Authentication required
--   500 Internal Server Error: Error occurred while querying industries
--
-- Authentication:
--   Required - Bearer token in Authorization header
--
-- Example Usage:
--   GET /api/v1/contacts/industry/
--   GET /api/v1/contacts/industry/?search=technology&separated=true
--   GET /api/v1/contacts/industry/?company=Bandura&separated=false
-- ============================================================================

-- ORM Implementation Notes:
--   The ContactRepository.list_industries_simple() uses conditional JOINs based on filters:
--   - When separated=true and PostgreSQL: Uses unnest(Company.industries) with conditional JOINs
--   - When separated=false: Uses array_to_string(Company.industries, ',') with conditional JOINs
--   - Always requires Company join (since selecting Company.industries)
...
-- GET /api/v1/contacts/industry/
-- Note: Always requires Company join since selecting Company.industries. Metadata joins only added when filters require them.
...
-- GET /api/v1/contacts/industry/?separated=false
SELECT DISTINCT array_to_string(co.industries, ',') as value
FROM contacts c
LEFT JOIN companies co ON c.company_id = co.uuid
...
-- GET /api/v1/contacts/industry/?distinct=true&separated=true
SELECT DISTINCT unnest(co.industries) as value
FROM contacts c
LEFT JOIN companies co ON c.company_id = co.uuid
LEFT JOIN contacts_metadata cm ON c.uuid = cm.uuid
...
-- GET /api/v1/contacts/industry/?distinct=false&separated=true
...
-- GET /api/v1/contacts/industry/?ordering=value&separated=true
SELECT DISTINCT unnest(co.industries) as value
FROM contacts c
LEFT JOIN companies co ON c.company_id = co.uuid
LEFT JOIN contacts_metadata cm ON c.uuid = cm.uuid
...
WHERE co.industries IS NOT NULL
    AND array_length(co.industries, 1) > 0
    AND EXISTS (
        SELECT 1 FROM unnest(co.industries) AS industry 
...
-- GET /api/v1/contacts/industry/?search=technology&separated=false
SELECT DISTINCT array_to_string(co.industries, ',') as value
FROM contacts c
LEFT JOIN companies co ON c.company_id = co.uuid
LEFT JOIN contacts_metadata cm ON c.uuid = cm.uuid
...
-- GET /api/v1/contacts/industry/?company=Bandura&separated=true
SELECT DISTINCT unnest(co.industries) as value
FROM contacts c
LEFT JOIN companies co ON c.company_id = co.uuid
LEFT JOIN contacts_metadata cm ON c.uuid = cm.uuid
...
-- GET /api/v1/contacts/industry/?limit=50&separated=true
SELECT DISTINCT unnest(co.industries) as value
FROM contacts c
LEFT JOIN companies co ON c.company_id = co.uuid
LEFT JOIN contacts_metadata cm ON c.uuid = cm.uuid
...
-- GET /api/v1/contacts/industry/?offset=25&separated=true
SELECT DISTINCT unnest(co.industries) as value
FROM contacts c
LEFT JOIN companies co ON c.company_id = co.uuid
LEFT JOIN contacts_metadata cm ON c.uuid = cm.uuid
...
-- GET /api/v1/contacts/industry/?limit=10&offset=20&separated=true
SELECT DISTINCT unnest(co.industries) as value
FROM contacts c
LEFT JOIN companies co ON c.company_id = co.uuid
LEFT JOIN contacts_metadata cm ON c.uuid = cm.uuid
...
-- GET /api/v1/contacts/industry/?search=technology&company=Bandura&separated=true
SELECT DISTINCT unnest(co.industries) as value
FROM contacts c
LEFT JOIN companies co ON c.company_id = co.uuid
LEFT JOIN contacts_metadata cm ON c.uuid = cm.uuid
...
-- GET /api/v1/contacts/industry/?distinct=true&ordering=-value&search=technology&separated=true
SELECT DISTINCT unnest(co.industries) as value
FROM contacts c
LEFT JOIN companies co ON c.company_id = co.uuid
LEFT JOIN contacts_metadata cm ON c.uuid = cm.uuid
...
-- GET /api/v1/contacts/industry/?distinct=true&limit=50&offset=0&ordering=value&search=technology&company=Bandura&separated=true
SELECT DISTINCT unnest(co.industries) as value
FROM contacts c
LEFT JOIN companies co ON c.company_id = co.uuid
LEFT JOIN contacts_metadata cm ON c.uuid = cm.uuid
...
-- GET /api/v1/contacts/industry/?first_name=Patrick&separated=true
SELECT DISTINCT unnest(co.industries) as value
FROM contacts c
LEFT JOIN companies co ON c.company_id = co.uuid
LEFT JOIN contacts_metadata cm ON c.uuid = cm.uuid
...
-- GET /api/v1/contacts/industry/?last_name=McGarry&separated=true
SELECT DISTINCT unnest(co.industries) as value
FROM contacts c
LEFT JOIN companies co ON c.company_id = co.uuid
LEFT JOIN contacts_metadata cm ON c.uuid = cm.uuid
...
-- GET /api/v1/contacts/industry/?seniority=C suite&separated=true
SELECT DISTINCT unnest(co.industries) as value
FROM contacts c
LEFT JOIN companies co ON c.company_id = co.uuid
LEFT JOIN contacts_metadata cm ON c.uuid = cm.uuid
...
-- GET /api/v1/contacts/industry/?email=banduracyber.com&separated=true
SELECT DISTINCT unnest(co.industries) as value
FROM contacts c
LEFT JOIN companies co ON c.company_id = co.uuid
LEFT JOIN contacts_metadata cm ON c.uuid = cm.uuid
...
-- GET /api/v1/contacts/industry/?department=C-Suite&separated=true
SELECT DISTINCT unnest(co.industries) as value
FROM contacts c
LEFT JOIN companies co ON c.company_id = co.uuid
LEFT JOIN contacts_metadata cm ON c.uuid = cm.uuid
...
-- GET /api/v1/contacts/industry/?employees_min=20&separated=true
SELECT DISTINCT unnest(co.industries) as value
FROM contacts c
LEFT JOIN companies co ON c.company_id = co.uuid
LEFT JOIN contacts_metadata cm ON c.uuid = cm.uuid
...
-- GET /api/v1/contacts/industry/?employees_max=100&separated=true
SELECT DISTINCT unnest(co.industries) as value
FROM contacts c
LEFT JOIN companies co ON c.company_id = co.uuid
LEFT JOIN contacts_metadata cm ON c.uuid = cm.uuid
...
-- GET /api/v1/contacts/industry/?employees_min=20&employees_max=100&separated=true
SELECT DISTINCT unnest(co.industries) as value
FROM contacts c
LEFT JOIN companies co ON c.company_id = co.uuid
LEFT JOIN contacts_metadata cm ON c.uuid = cm.uuid
...
-- Query 25: With ContactFilterParams - technologies filter and separated=true
-- GET /api/v1/contacts/industry/?technologies=Salesforce&separated=true
SELECT DISTINCT unnest(co.industries) as value
FROM contacts c
LEFT JOIN companies co ON c.company_id = co.uuid
LEFT JOIN contacts_metadata cm ON c.uuid = cm.uuid
...
-- GET /api/v1/contacts/industry/?keywords=cyber security&separated=true
SELECT DISTINCT unnest(co.industries) as value
FROM contacts c
LEFT JOIN companies co ON c.company_id = co.uuid
LEFT JOIN contacts_metadata cm ON c.uuid = cm.uuid
...
-- GET /api/v1/contacts/industry/?city=Miamisburg&separated=true
SELECT DISTINCT unnest(co.industries) as value
FROM contacts c
LEFT JOIN companies co ON c.company_id = co.uuid
LEFT JOIN contacts_metadata cm ON c.uuid = cm.uuid
...
-- GET /api/v1/contacts/industry/?state=Ohio&separated=true
SELECT DISTINCT unnest(co.industries) as value
FROM contacts c
LEFT JOIN companies co ON c.company_id = co.uuid
LEFT JOIN contacts_metadata cm ON c.uuid = cm.uuid
...
-- GET /api/v1/contacts/industry/?country=United States&separated=true
SELECT DISTINCT unnest(co.industries) as value
FROM contacts c
LEFT JOIN companies co ON c.company_id = co.uuid
LEFT JOIN contacts_metadata cm ON c.uuid = cm.uuid
...
-- GET /api/v1/contacts/industry/?annual_revenue_min=7000000&separated=true
SELECT DISTINCT unnest(co.industries) as value
FROM contacts c
LEFT JOIN companies co ON c.company_id = co.uuid
LEFT JOIN contacts_metadata cm ON c.uuid = cm.uuid
...
-- GET /api/v1/contacts/industry/?annual_revenue_max=9000000&separated=true
SELECT DISTINCT unnest(co.industries) as value
FROM contacts c
LEFT JOIN companies co ON c.company_id = co.uuid
LEFT JOIN contacts_metadata cm ON c.uuid = cm.uuid
...
-- Query 32: With ContactFilterParams - exclude_industries filter and separated=true
-- GET /api/v1/contacts/industry/?exclude_industries=Retail,Agriculture&separated=true
SELECT DISTINCT unnest(co.industries) as value
FROM contacts c
LEFT JOIN companies co ON c.company_id = co.uuid
LEFT JOIN contacts_metadata cm ON c.uuid = cm.uuid
...
WHERE co.industries IS NOT NULL
    AND array_length(co.industries, 1) > 0
    AND NOT EXISTS (
        SELECT 1 FROM unnest(ARRAY['Retail', 'Agriculture']) AS exclude_val 
        WHERE array_to_string(co.industries, ',') ILIKE '%' || exclude_val || '%'
    )
ORDER BY value ASC
LIMIT 25
OFFSET 0;
...
-- GET /api/v1/contacts/industry/?exclude_company_ids=398cce44-233d-5f7c-aea1-e4a6a79df10c&separated=true
SELECT DISTINCT unnest(co.industries) as value
FROM contacts c
LEFT JOIN companies co ON c.company_id = co.uuid
LEFT JOIN contacts_metadata cm ON c.uuid = cm.uuid
...
-- GET /api/v1/contacts/industry/?created_at_after=2024-01-01T00:00:00&separated=true
SELECT DISTINCT unnest(co.industries) as value
FROM contacts c
LEFT JOIN companies co ON c.company_id = co.uuid
LEFT JOIN contacts_metadata cm ON c.uuid = cm.uuid
...
-- GET /api/v1/contacts/industry/?created_at_before=2024-12-31T23:59:59&separated=true
SELECT DISTINCT unnest(co.industries) as value
FROM contacts c
LEFT JOIN companies co ON c.company_id = co.uuid
LEFT JOIN contacts_metadata cm ON c.uuid = cm.uuid
...
-- GET /api/v1/contacts/industry/?company=Bandura&seniority=C suite&employees_min=20&employees_max=100&separated=true
SELECT DISTINCT unnest(co.industries) as value
FROM contacts c
LEFT JOIN companies co ON c.company_id = co.uuid
LEFT JOIN contacts_metadata cm ON c.uuid = cm.uuid
...
-- GET /api/v1/contacts/industry/?search=technology&company=Bandura&distinct=true&ordering=-value&separated=true
SELECT DISTINCT unnest(co.industries) as value
FROM contacts c
LEFT JOIN companies co ON c.company_id = co.uuid
LEFT JOIN contacts_metadata cm ON c.uuid = cm.uuid
...
-- GET /api/v1/contacts/industry/?search=technology&company=Bandura&separated=false
SELECT DISTINCT array_to_string(co.industries, ',') as value
FROM contacts c
LEFT JOIN companies co ON c.company_id = co.uuid
LEFT JOIN contacts_metadata cm ON c.uuid = cm.uuid
...
-- GET /api/v1/contacts/industry/?distinct=true&limit=50&offset=0&ordering=value&search=technology&company=Bandura&seniority=C suite&employees_min=20&separated=true
SELECT DISTINCT unnest(co.industries) as value
FROM contacts c
LEFT JOIN companies co ON c.company_id = co.uuid
LEFT JOIN contacts_metadata cm ON c.uuid = cm.uuid
