...
-- API Version: v1
-- Description: Return distinct values for /contacts/company/ using AttributeListParams.
-- ============================================================================
--
-- Parameters:
...
-- Query 1: Basic query - Get all distinct companies
...
-- GET /api/v1/contacts/company/?search=Bandura
SELECT DISTINCT co.name as value
FROM contacts c
LEFT JOIN companies co ON c.company_id = co.uuid
LEFT JOIN contacts_metadata cm ON c.uuid = cm.uuid
...
-- GET /api/v1/contacts/company/?company=Bandura
SELECT DISTINCT co.name as value
FROM contacts c
LEFT JOIN companies co ON c.company_id = co.uuid
LEFT JOIN contacts_metadata cm ON c.uuid = cm.uuid
...
-- GET /api/v1/contacts/company/?limit=10&offset=20
SELECT DISTINCT co.name as value
FROM contacts c
LEFT JOIN companies co ON c.company_id = co.uuid
LEFT JOIN contacts_metadata cm ON c.uuid = cm.uuid
...
-- GET /api/v1/contacts/company/?search=Bandura&company=Bandura
SELECT DISTINCT co.name as value
FROM contacts c
LEFT JOIN companies co ON c.company_id = co.uuid
LEFT JOIN contacts_metadata cm ON c.uuid = cm.uuid
...
-- GET /api/v1/contacts/company/?distinct=true&ordering=-value&search=Bandura
SELECT DISTINCT co.name as value
FROM contacts c
LEFT JOIN companies co ON c.company_id = co.uuid
LEFT JOIN contacts_metadata cm ON c.uuid = cm.uuid
...
-- GET /api/v1/contacts/company/?distinct=true&limit=50&offset=0&ordering=value&search=Bandura&company=Bandura
SELECT DISTINCT co.name as value
FROM contacts c
LEFT JOIN companies co ON c.company_id = co.uuid
LEFT JOIN contacts_metadata cm ON c.uuid = cm.uuid
...
-- GET /api/v1/contacts/company/?first_name=Patrick
SELECT DISTINCT co.name as value
FROM contacts c
LEFT JOIN companies co ON c.company_id = co.uuid
LEFT JOIN contacts_metadata cm ON c.uuid = cm.uuid
...
-- GET /api/v1/contacts/company/?last_name=McGarry
SELECT DISTINCT co.name as value
FROM contacts c
LEFT JOIN companies co ON c.company_id = co.uuid
LEFT JOIN contacts_metadata cm ON c.uuid = cm.uuid
...
-- GET /api/v1/contacts/company/?title=Chief Technology Officer
SELECT DISTINCT co.name as value
FROM contacts c
LEFT JOIN companies co ON c.company_id = co.uuid
LEFT JOIN contacts_metadata cm ON c.uuid = cm.uuid
...
-- GET /api/v1/contacts/company/?seniority=C suite
SELECT DISTINCT co.name as value
FROM contacts c
LEFT JOIN companies co ON c.company_id = co.uuid
LEFT JOIN contacts_metadata cm ON c.uuid = cm.uuid
...
-- GET /api/v1/contacts/company/?email=banduracyber.com
SELECT DISTINCT co.name as value
FROM contacts c
LEFT JOIN companies co ON c.company_id = co.uuid
LEFT JOIN contacts_metadata cm ON c.uuid = cm.uuid
...
-- GET /api/v1/contacts/company/?department=C-Suite
SELECT DISTINCT co.name as value
FROM contacts c
LEFT JOIN companies co ON c.company_id = co.uuid
LEFT JOIN contacts_metadata cm ON c.uuid = cm.uuid
...
-- GET /api/v1/contacts/company/?employees_min=20
SELECT DISTINCT co.name as value
FROM contacts c
LEFT JOIN companies co ON c.company_id = co.uuid
LEFT JOIN contacts_metadata cm ON c.uuid = cm.uuid
...
-- GET /api/v1/contacts/company/?employees_max=100
SELECT DISTINCT co.name as value
FROM contacts c
LEFT JOIN companies co ON c.company_id = co.uuid
LEFT JOIN contacts_metadata cm ON c.uuid = cm.uuid
...
-- GET /api/v1/contacts/company/?employees_min=20&employees_max=100
SELECT DISTINCT co.name as value
FROM contacts c
LEFT JOIN companies co ON c.company_id = co.uuid
LEFT JOIN contacts_metadata cm ON c.uuid = cm.uuid
...
-- GET /api/v1/contacts/company/?industries=information technology
SELECT DISTINCT co.name as value
FROM contacts c
LEFT JOIN companies co ON c.company_id = co.uuid
LEFT JOIN contacts_metadata cm ON c.uuid = cm.uuid
...
-- GET /api/v1/contacts/company/?technologies=Salesforce
SELECT DISTINCT co.name as value
FROM contacts c
LEFT JOIN companies co ON c.company_id = co.uuid
LEFT JOIN contacts_metadata cm ON c.uuid = cm.uuid
...
-- GET /api/v1/contacts/company/?keywords=cyber security
SELECT DISTINCT co.name as value
FROM contacts c
LEFT JOIN companies co ON c.company_id = co.uuid
LEFT JOIN contacts_metadata cm ON c.uuid = cm.uuid
...
-- GET /api/v1/contacts/company/?city=Miamisburg
SELECT DISTINCT co.name as value
FROM contacts c
LEFT JOIN companies co ON c.company_id = co.uuid
LEFT JOIN contacts_metadata cm ON c.uuid = cm.uuid
...
-- GET /api/v1/contacts/company/?state=Ohio
SELECT DISTINCT co.name as value
FROM contacts c
LEFT JOIN companies co ON c.company_id = co.uuid
LEFT JOIN contacts_metadata cm ON c.uuid = cm.uuid
...
-- GET /api/v1/contacts/company/?country=United States
SELECT DISTINCT co.name as value
FROM contacts c
LEFT JOIN companies co ON c.company_id = co.uuid
LEFT JOIN contacts_metadata cm ON c.uuid = cm.uuid
...
-- GET /api/v1/contacts/company/?annual_revenue_min=7000000
SELECT DISTINCT co.name as value
FROM contacts c
LEFT JOIN companies co ON c.company_id = co.uuid
LEFT JOIN contacts_metadata cm ON c.uuid = cm.uuid
...
-- GET /api/v1/contacts/company/?annual_revenue_max=9000000
SELECT DISTINCT co.name as value
FROM contacts c
LEFT JOIN companies co ON c.company_id = co.uuid
LEFT JOIN contacts_metadata cm ON c.uuid = cm.uuid
...
-- GET /api/v1/contacts/company/?exclude_company_ids=398cce44-233d-5f7c-aea1-e4a6a79df10c
SELECT DISTINCT co.name as value
FROM contacts c
LEFT JOIN companies co ON c.company_id = co.uuid
LEFT JOIN contacts_metadata cm ON c.uuid = cm.uuid
...
-- GET /api/v1/contacts/company/?created_at_after=2024-01-01T00:00:00
SELECT DISTINCT co.name as value
FROM contacts c
LEFT JOIN companies co ON c.company_id = co.uuid
LEFT JOIN contacts_metadata cm ON c.uuid = cm.uuid
...
-- GET /api/v1/contacts/company/?created_at_before=2024-12-31T23:59:59
SELECT DISTINCT co.name as value
FROM contacts c
LEFT JOIN companies co ON c.company_id = co.uuid
LEFT JOIN contacts_metadata cm ON c.uuid = cm.uuid
...
-- Query 34: With ContactFilterParams - updated_at_after filter
-- GET /api/v1/contacts/company/?updated_at_after=2024-06-01T00:00:00
SELECT DISTINCT co.name as value
FROM contacts c
LEFT JOIN companies co ON c.company_id = co.uuid
LEFT JOIN contacts_metadata cm ON c.uuid = cm.uuid
...
-- Query 35: With ContactFilterParams - updated_at_before filter
-- GET /api/v1/contacts/company/?updated_at_before=2024-10-01T00:00:00
SELECT DISTINCT co.name as value
FROM contacts c
LEFT JOIN companies co ON c.company_id = co.uuid
LEFT JOIN contacts_metadata cm ON c.uuid = cm.uuid
...
-- Query 36: With multiple ContactFilterParams combined
-- GET /api/v1/contacts/company/?company=Bandura&seniority=C suite&employees_min=20&employees_max=100
SELECT DISTINCT co.name as value
FROM contacts c
LEFT JOIN companies co ON c.company_id = co.uuid
LEFT JOIN contacts_metadata cm ON c.uuid = cm.uuid
...
-- Query 37: With search parameter and ContactFilterParams
-- GET /api/v1/contacts/company/?search=Bandura&company=Bandura&distinct=true&ordering=-value
SELECT DISTINCT co.name as value
FROM contacts c
LEFT JOIN companies co ON c.company_id = co.uuid
LEFT JOIN contacts_metadata cm ON c.uuid = cm.uuid
...
-- GET /api/v1/contacts/company/?company=Bandura&seniority=C suite&employees_min=20&city=Miamisburg&limit=50
SELECT DISTINCT co.name as value
FROM contacts c
LEFT JOIN companies co ON c.company_id = co.uuid
LEFT JOIN contacts_metadata cm ON c.uuid = cm.uuid
...
-- GET /api/v1/contacts/company/?distinct=true&limit=50&offset=0&ordering=value&search=Bandura&company=Bandura&seniority=C suite&employees_min=20
SELECT DISTINCT co.name as value
FROM contacts c
LEFT JOIN companies co ON c.company_id = co.uuid
LEFT JOIN contacts_metadata cm ON c.uuid = cm.uuid
...
-- GET /api/v1/contacts/company/?mobile_phone=937-555
SELECT DISTINCT co.name as value
FROM contacts c
LEFT JOIN companies co ON c.company_id = co.uuid
LEFT JOIN contacts_metadata cm ON c.uuid = cm.uuid
