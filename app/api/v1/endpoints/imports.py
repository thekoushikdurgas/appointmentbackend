        ...
        async with aiofiles.open(temp_path, "wb") as async_file:
            while True:
                try:
                    ...
                    # Log progress periodically (every 10MB or 100 chunks)
                    if chunks_written % 100 == 0 or total_bytes % (10 * 1024 * 1024) == 0:
                        logger.debug(
                    ...
                    await async_file.close()
                    temp_path.unlink(missing_ok=True)
                    logger.warning("Upload cancelled: filename=%s bytes_uploaded=%d", file.filename, total_bytes)
                    raise
                    ...
                    temp_path.unlink(missing_ok=True)
                    logger.exception(
                        "Error during chunked upload: filename=%s chunk=%d bytes=%d error=%s",
                        file.filename,
                        chunks_written,
                        total_bytes,
                        chunk_exc,
