...
class RedisQueryCache:
    ...
    async def _get_client(self):
        ...
        if self._redis_client is None:
            try:
                ...
                self.logger.info("Redis cache connected: url=%s", self.redis_url)
            except Exception as exc:
                self.logger.error("Failed to connect to Redis: %s", exc)
    ...
    async def get(self, prefix: str, *args, **kwargs) -> Optional[Any]:
        ...
        try:
            ...
            if value is None:
                self.logger.debug("Cache miss (Redis) for key=%s", cache_key)
                return None
            
            # Deserialize JSON value
            result = json.loads(value)
            self.logger.debug("Cache hit (Redis) for key=%s", cache_key)
            return result
        except Exception as exc:
            self.logger.warning("Error reading from Redis cache: %s", exc)
            return None
    ...
    async def set(
        ...
        **kwargs,
        ...
        try:
            ...
            self.logger.debug("Cached result (Redis) for key=%s ttl=%d", cache_key, ttl)
            return True
        except Exception as exc:
            self.logger.warning("Error writing to Redis cache: %s", exc)
            return False
    ...
    async def delete(self, prefix: str, *args, **kwargs) -> bool:
        ...
        try:
            ...
            if deleted:
                self.logger.debug("Deleted cache key (Redis) key=%s", cache_key)
            return deleted > 0
        except Exception as exc:
            self.logger.warning("Error deleting from Redis cache: %s", exc)
            return False
    ...
    async def clear_prefix(self, prefix: str) -> int:
        ...
        try:
            ...
            if deleted > 0:
                self.logger.debug("Cleared %d cache entries (Redis) with prefix=%s", deleted, prefix)
            return deleted
        except Exception as exc:
            self.logger.warning("Error clearing Redis cache prefix: %s", exc)
            return 0
    ...
    async def invalidate_pattern(self, pattern: str) -> int:
        ...
        try:
            ...
            if deleted > 0:
                self.logger.debug("Invalidated %d cache entries (Redis) matching pattern=%s", deleted, pattern)
            return deleted
        except Exception as exc:
            self.logger.warning("Error invalidating Redis cache pattern: %s", exc)
            return 0
    ...
    async def get_cache_stats(self) -> dict[str, Any]:
        ...
        try:
            ...
            self.logger.warning("Error getting Redis cache stats: %s", exc)
...
class QueryCache:
    ...
    def __init__(self, enabled: bool = True, use_redis: bool = False, redis_url: Optional[str] = None):
        ...
        if use_redis and redis_url and settings.ENABLE_REDIS_CACHE:
            if REDIS_AVAILABLE:
                ...
                self.logger.info("Using Redis backend for query cache")
            else:
                self.logger.warning("Redis requested but not available. Falling back to in-memory cache.")
    ...
    def _init_in_memory_cache(self) -> None:
        ...
        if not CACHETOOLS_AVAILABLE:
            self.logger.warning(
                "cachetools not available. Install with: pip install cachetools>=5.3.0. "
        ...
        self.logger.info(
            "Initialized in-memory cache: maxsize=%d ttl=%ds (cachetools.TTLCache)",
            maxsize,
            self.default_ttl,
        )
    ...
    async def get(self, prefix: str, *args, **kwargs) -> Optional[Any]:
        ...
        try:
            ...
            async with self._lock:
                ...
                # TTLCache automatically handles expiration - if key exists, it's valid
...
async def invalidate_pattern_safe(
    ...
    try:
        ...
        if count > 0:
            cache.logger.info("Invalidated %d cache entries matching pattern: %s", count, pattern)
        return count
    except Exception as exc:
        cache.logger.warning("Error invalidating cache pattern %s: %s", pattern, exc)
        return 0
...
def register_invalidation_handler(
    ...
    Example:
        async def log_invalidation(event: CacheInvalidationEvent):
            logger.info("Cache invalidated: prefix=%s reason=%s", event.prefix, event.reason)
    ...
    logger.debug("Registered invalidation handler: prefix=%s", prefix)
    ...
    **kwargs,
    ...
    for handler in handlers:
        try:
            ...
            logger.warning(
                "Error in invalidation handler: prefix=%s handler=%s error=%s",
                prefix,
                handler.__name__,
                exc,
            )
    ...
    logger.debug(
        "Cache invalidation event: prefix=%s pattern=%s reason=%s count=%d",
        prefix,
        pattern,
        reason,
        count,
...
async def warm_cache_entry(
    ...
    **kwargs,
    ...
    value = await fetch_func(*args, **kwargs)
    await cache.set(prefix, value, ttl=ttl, *args, **kwargs)
    logger.debug("Warmed cache entry: prefix=%s", prefix)
    return value
...
async def warm_cache_batch(
    ...
    for task in warming_tasks:
        try:
            ...
            logger.warning("Failed to warm cache entry: %s", exc)
    ...
    logger.info(
        "Cache warming batch completed: successful=%d failed=%d total=%d",
        successful,
        failed,
        len(warming_tasks),
    )
