...
async def get_cached_list_result(
    ...
    try:
        ...
        if cached_result:
            log.debug("Cache hit for %s list query", cache_prefix)
            return cached_result
    except Exception as exc:
        log.warning("Error checking cache for %s list: %s", cache_prefix, exc)
...
async def cache_list_result(
    ...
    try:
        ...
        log.warning("Failed to cache %s list result: %s", cache_prefix, exc)
...
async def execute_list_query(
    ...
    cached_result = await get_cached_list_result(
        cache_prefix, filters, limit, offset, use_cursor, log
    )
    if cached_result:
    ...
    # Cache result
    await cache_list_result(cache_prefix, page, filters, limit, offset, use_cursor, log)
