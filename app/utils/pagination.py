...
def build_paginated_attribute_list(
    ...
    log.debug(
        ...
        params.limit,
        modified_limit if params.limit is not None else None,
        raw_count,
        has_more_results,
    )
    ...
    if distinct_requested:
        ...
        log.debug(
            "Deduplicated values: before=%d after=%d",
            len(values),
            len(unique_values),
        )
...
def normalize_page_size(
    ...
    if requested <= 0:
        logger.warning("Invalid page size requested: %d, using default: %s", requested, default)
        return default
    
    if max_size is not None and requested > max_size:
        logger.warning(
            "Page size %d exceeds maximum %d, capping to %d",
            requested,
            max_size,
            max_size,
        )
...
def apply_pagination_to_query(
    ...
    if offset > 1000 and use_index_hint:
        logger.debug(
            "Large offset detected (%d). Ensure ordering column is indexed for optimal performance.",
            offset,
        )
