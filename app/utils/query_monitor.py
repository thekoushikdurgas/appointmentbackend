            ...
            def after_cursor_execute(conn, cursor, statement, parameters, context, executemany):
                ...
                if hasattr(context, "_query_start_time"):
                    ...
                    if query_time > self.slow_query_threshold:
                        ...
                        self.logger.warning(
                            "Slow query detected: time=%.3fs statement=%s parameters=%s",
                            query_time,
                            statement[:200] if statement else None,
                            str(parameters)[:200] if parameters else None,
                        )
                        
                        # For very slow queries (>5 seconds), suggest query plan analysis
                        if query_time > 5.0 and statement:
                            # Log a truncated version of the query for manual EXPLAIN analysis
                            query_preview = statement[:500] if statement else "N/A"
                            self.logger.warning(
                                "Very slow query detected (%.3fs). Consider running EXPLAIN ANALYZE manually. Query preview: %s",
                                query_time,
                                query_preview,
                            )
                    elif settings.DATABASE_ECHO:
                        self.logger.debug(
                            "Query executed: time=%.3fs",
                            query_time,
                        )

        except AttributeError:
            self.logger.debug("Could not set up query monitoring (sync_engine not available)")
...
async def measure_query_time(operation_name: str):
    ...
    try:
        ...
        if elapsed > SLOW_QUERY_THRESHOLD:
            logger.warning(
                "Slow operation detected: operation=%s time=%.3fs",
                operation_name,
                elapsed,
            )
        else:
            logger.debug(
                "Operation completed: operation=%s time=%.3fs",
                operation_name,
                elapsed,
            )
