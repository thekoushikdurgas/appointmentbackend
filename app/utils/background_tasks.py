        ...
        try:
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    logger.info(
                        "Background task completed: func=%s duration=%.3fs attempt=%d cpu_bound=%s",
                        func.__name__,
                        duration,
                        attempt + 1,
                        is_cpu_bound,
                    ...
                    logger.warning(
                        "Background task failed: func=%s attempt=%d/%d error=%s error_type=%s",
                        func.__name__,
                        attempt + 1,
                        max_retries,
                        str(exc),
            ...
            logger.error(
                "Background task failed after %d attempts: func=%s duration=%.3fs error=%s error_type=%s",
                max_retries,
                func.__name__,
                duration,
                last_exception,
    ...
    **kwargs,
    ...
    async def safe_task():
        ...
        try:
            ...
            logger.info(
                "Background task completed: func=%s duration=%.3fs cpu_bound=%s",
                func.__name__,
                duration,
                is_cpu_bound,
            )
            ...
            logger.exception(
                "Background task failed (non-critical): func=%s duration=%.3fs error=%s error_type=%s",
                func.__name__,
                duration,
                exc,
                error_type,
...
async def wait_for_active_tasks(timeout: Optional[float] = None) -> bool:
    ...
    logger.info("Waiting for %d active background tasks to complete (timeout=%.1fs)...", len(tasks), timeout)
    try:
        ...
        logger.warning(
            "Timeout waiting for background tasks to complete: %d tasks still running after %.1fs",
            len(tasks),
            timeout,
        )
        return False
    ...
    async def chained_executor():
        ...
        for func, args, kwargs in task_functions:
            try:
                ...
                logger.error("Error in chained task %s: %s", func.__name__, exc, exc_info=True)
    ...
    async def task_with_deps():
        ...
        for dep_func in dependencies:
            try:
                ...
                logger.error("Dependency task %s failed: %s", dep_func.__name__, exc, exc_info=True)
                raise
    ...
    **kwargs,
    ...
    async for session in db_session_factory():
        try:
            ...
            logger.exception("Error in background task with DB session: %s", exc)
            raise
        finally:
...
async def store_task_result(task_id: str, result: Any, ttl: Optional[int] = None) -> None:
    ...
    logger.debug("Stored task result: task_id=%s", task_id)
