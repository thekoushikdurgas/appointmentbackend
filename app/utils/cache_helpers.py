        ...
        if key_func is None:
            ...
            def wrapper(*args, **kwargs) -> T:
                ...
                if cache_key in cache:
                    logger.debug("Cache hit (lru_cache): func=%s key=%s", func.__name__, cache_key)
                ...
                if maxsize is not None and len(cache) >= maxsize:
                    ...
                    logger.debug("Cache evicted (lru_cache): func=%s key=%s", func.__name__, oldest_key)
                
                cache[cache_key] = result
                logger.debug("Cache miss (lru_cache): func=%s key=%s", func.__name__, cache_key)
                return result
...
def get_reference_data_cached(data_type: str) -> dict[str, Any]:
    ...
    logger.debug("Fetching reference data: type=%s", data_type)
    return {}
...
def clear_all_lru_caches() -> dict[str, int]:
    ...
    logger.info("Cleared lru_cache functions: %s", cleared)
    return cleared
