...
async def batch_fetch_companies_by_uuids(
    ...
    logger.debug("Batch fetching %d companies by UUIDs", len(company_uuids))
    ...
    logger.debug("Fetched %d companies out of %d requested", len(result_dict), len(company_uuids))
    return result_dict
...
async def batch_fetch_contact_metadata_by_uuids(
    ...
    logger.debug("Batch fetching contact metadata for %d contacts", len(contact_uuids))
    ...
    logger.debug("Fetched %d contact metadata records out of %d requested", len(result_dict), len(contact_uuids))
    return result_dict
...
async def batch_fetch_company_metadata_by_uuids(
    ...
    logger.debug("Batch fetching company metadata for %d companies", len(company_uuids))
    ...
    logger.debug("Fetched %d company metadata records out of %d requested", len(result_dict), len(company_uuids))
    return result_dict
...
async def fetch_company_by_uuid(
    ...
    logger.debug("Fetching company by UUID: %s", company_uuid)
...
async def fetch_contact_metadata_by_uuid(
    ...
    """
    logger.debug("Fetching contact metadata by UUID: %s", contact_uuid)
...
async def fetch_company_metadata_by_uuid(
    ...
    """
    logger.debug("Fetching company metadata by UUID: %s", company_uuid)
