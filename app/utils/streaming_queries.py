...
async def stream_query_results(
    ...
    if batch_size > 10000:
        logger.warning("Large batch_size=%d may cause memory issues. Consider using smaller batches.", batch_size)
    ...
    try:
        while True:
            ...
            try:
                ...
                logger.exception(
                    "Error executing streaming query batch: offset=%d consecutive_errors=%d",
                    offset,
                    consecutive_errors,
                )
                ...
                if consecutive_errors >= max_consecutive_errors:
                    logger.error(
                        "Too many consecutive errors (%d), aborting stream at offset=%d",
                        consecutive_errors,
                        offset,
                    )
                    raise
            ...
            logger.debug("Fetched batch: offset=%d size=%d total_fetched=%d", offset, len(batch), total_fetched)
            
            try:
                yield batch
            except GeneratorExit:
                # Consumer stopped iterating - cleanup and exit
                logger.debug("Stream interrupted by consumer: total_fetched=%d", total_fetched)
                break
            except Exception as exc:
                # Error in consumer - log but continue
                logger.warning("Error in stream consumer: %s", exc, exc_info=True)
        ...
        logger.error(
            "Fatal error in query stream: offset=%d total_fetched=%d error=%s",
            offset,
            total_fetched,
            exc,
            exc_info=True,
        ...
        logger.debug("Query stream finished: total_fetched=%d", total_fetched)
    ...
    try:
        while True:
            ...
            try:
                ...
                consecutive_errors += 1
                logger.exception(
                    "Error executing streaming scalar query batch: offset=%d consecutive_errors=%d",
                    offset,
                    consecutive_errors,
                )
                ...
                if consecutive_errors >= max_consecutive_errors:
                    logger.error(
                        "Too many consecutive errors (%d), aborting scalar stream at offset=%d",
                        consecutive_errors,
                        offset,
                    )
                    raise
            ...
            logger.debug("Fetched scalar batch: offset=%d size=%d total_fetched=%d", offset, len(batch), total_fetched)
            
            try:
                yield batch
            except GeneratorExit:
                logger.debug("Scalar stream interrupted by consumer: total_fetched=%d", total_fetched)
                break
            except Exception as exc:
                logger.warning("Error in scalar stream consumer: %s", exc, exc_info=True)
        ...
        logger.error(
            "Fatal error in scalar query stream: offset=%d total_fetched=%d error=%s",
            offset,
            total_fetched,
            exc,
            exc_info=True,
        ...
        logger.debug("Scalar query stream finished: total_fetched=%d", total_fetched)
