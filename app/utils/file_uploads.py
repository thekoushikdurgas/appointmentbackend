...
def calculate_optimal_chunk_size(
    ...
    logger.debug(
        "Calculated optimal chunk size: file_size=%s file_type=%s network=%s chunk_size=%d",
        file_size,
        file_type,
        network_condition,
        optimal_size,
...
async def save_upload_file_chunked(
    ...
    try:
        async with aiofiles.open(destination, "wb") as async_file:
            while True:
                ...
                if chunks_written % 100 == 0:
                    logger.debug(
                        "Upload progress: filename=%s bytes=%d chunks=%d",
                        file.filename,
                        total_bytes,
                        chunks_written,
                    )
        ...
        logger.info(
            "File uploaded successfully: filename=%s size=%d chunks=%d duration=%.2fs hash=%s",
            file.filename,
            total_bytes,
            chunks_written,
            duration,
        ...
        destination.unlink(missing_ok=True)
        logger.exception("Failed to save uploaded file: filename=%s error=%s", file.filename, exc)
        raise
...
class ResumableUpload:
    ...
    async def _continue_upload(
        ...
        try:
            ...
            logger.exception("Failed to continue upload: upload_id=%s error=%s", self.upload_id, exc)
            raise
