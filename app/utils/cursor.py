...
def encode_keyset_cursor(last_id: int, last_value: Optional[Any] = None) -> str:
    ...
    """
    logger.debug("Entering encode_keyset_cursor last_id=%d", last_id)
    cursor_data = {"id": last_id}
    if last_value is not None:
        cursor_data["value"] = last_value
    token = json.dumps(cursor_data)
    encoded = base64.urlsafe_b64encode(token.encode("utf-8")).decode("utf-8")
    logger.debug("Exiting encode_keyset_cursor result=%s", encoded)
    return encoded
...
def decode_keyset_cursor(token: str) -> tuple[int, Optional[Any]]:
    ...
    """
    logger.debug("Entering decode_keyset_cursor token=%s", token)
    try:
        decoded = base64.urlsafe_b64decode(token.encode("utf-8")).decode("utf-8")
        ...
        logger.debug("Exiting decode_keyset_cursor last_id=%d", last_id)
        return int(last_id), last_value
    except Exception as exc:
        logger.warning("Failed to decode keyset cursor: token=%s error=%s", token, exc)
