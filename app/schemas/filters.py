...
class ContactFilterParams(BaseModel):
    ...
    include_domain_list: Optional[list[str]] = Field(
        default=None,
        description="Include contacts whose company website domain matches any provided domain (case-insensitive). Domains are extracted from CompanyMetadata.website.",
    )
    exclude_domain_list: Optional[list[str]] = Field(
        default=None,
        description="Exclude contacts whose company website domain matches any provided domain (case-insensitive). Domains are extracted from CompanyMetadata.website.",
    )
...
CONTACT_FILTER_COLUMN_MAP: dict[str, str] = {
    ...
    "include_domain_list": "CompanyMetadata.website (domain inclusion list)",
...
class FilterDataRequest(BaseModel):
    """Request schema for filter data endpoint."""
    
    service: str = Field(..., description="Service name: 'contact' or 'company'")
    filter_key: str = Field(..., description="Filter key from filters list")
    search_text: Optional[str] = Field(None, description="Optional text to filter results")
