...
class EmailFinderRepository(AsyncRepository):
    ...
    async def find_emails_by_name_and_domain(
        ...
        # WARNING: This query can take 20+ seconds on large tables. Consider populating normalized_domain
        # column for all companies to avoid this slow fallback.
        website_uuids = []
        website_count = 0
        if normalized_count == 0:
            logger.warning(
                "Step 1 Strategy 2: normalized_domain found nothing for domain=%s, "
                "trying slow website extraction (this may take 20+ seconds)",
                normalized_domain
            )
            website_domain_expression = self._extract_domain_from_website_sql(
                CompanyMetadata.website,
            ...
            ).limit(1000)  # Add limit to prevent extremely long queries
        ...
        if step1_count != step2_count:
            orphaned_count = step1_count - step2_count
            logger.warning(
                "Step 1/2 mismatch: companies_metadata has %d UUIDs but companies table has %d UUIDs (domain=%s). "
                "This indicates %d orphaned records in companies_metadata. Using only validated UUIDs from companies table.",
                step1_count,
                step2_count,
                normalized_domain,
                orphaned_count,
            )
            # Continue with validated UUIDs from companies table (step2_uuids)
            # This gracefully handles the mismatch by using only valid company UUIDs
            # Note: Orphaned records in companies_metadata should be cleaned up via a maintenance script
