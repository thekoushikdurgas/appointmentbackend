...
class TokenBlacklistRepository(AsyncRepository[TokenBlacklist]):
    ...
    def __init__(self) -> None:
        ...
        logger.debug("Entering TokenBlacklistRepository.__init__")
        super().__init__(TokenBlacklist)
        logger.debug("Exiting TokenBlacklistRepository.__init__")
    ...
    async def create_blacklist_entry(
        ...
        """
        logger.debug("Creating blacklist entry: user_id=%s", user_id)
        ...
        logger.debug("Created blacklist entry: token=%s... user_id=%s", token[:20], user_id)
    ...
    async def is_token_blacklisted(
        ...
        """
        logger.debug("Checking if token is blacklisted: token=%s...", token[:20])
        ...
        logger.debug("Token blacklist check: token=%s... is_blacklisted=%s", token[:20], is_blacklisted)
    ...
    async def cleanup_expired_tokens(
        ...
        await session.flush()
        logger.info("Cleaned up %d expired blacklist tokens", count)
        return count
    ...
    async def get_by_token(
        ...
        logger.debug("Getting blacklist entry by token: token=%s...", token[:20])
        stmt: Select[tuple[TokenBlacklist]] = select(self.model).where(self.model.token == token)
        result = await session.execute(stmt)
        entry = result.scalar_one_or_none()
        logger.debug("Blacklist entry %sfound for token=%s...", "" if entry else "not ", token[:20])
        return entry
