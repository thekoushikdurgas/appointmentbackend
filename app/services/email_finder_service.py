...
class EmailFinderService:
    ...
    async def find_emails(
        ...
        if self.email_finder_repo is None:
            self.logger.error("EmailFinderRepository is None, cannot proceed with email search")
            raise HTTPException(
        ...
        if not hasattr(self.email_finder_repo, 'find_emails_by_name_and_domain'):
            self.logger.error("EmailFinderRepository missing find_emails_by_name_and_domain method")
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
