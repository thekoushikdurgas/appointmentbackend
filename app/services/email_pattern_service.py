...
class EmailPatternService:
    ...
    def __init__(
        ...
        if self.email_pattern_repo is None:
            raise ValueError("EmailPatternRepository cannot be None")
        if self.company_repo is None:
            raise ValueError("CompanyRepository cannot be None")
        self.logger.debug("EmailPatternService initialized")
                ...
                if upsert:
                    ...
                    if updated_pattern:
                        ...
                        self.logger.info(
                            ...
                            updated_pattern.uuid,
                            updated_pattern.company_uuid,
                            updated_pattern.contact_count,
                        )
    ...
    async def get_patterns_by_company(
        ...
        self.logger.debug("Getting patterns for company: company_uuid=%s", company_uuid)
    ...
    async def update_pattern(
        ...
        self.logger.debug("Updating email pattern: pattern_uuid=%s", pattern_uuid)
        ...
        self.logger.info("Updated email pattern: pattern_uuid=%s", pattern_uuid)
    ...
    async def delete_pattern(
        ...
        self.logger.debug("Deleting email pattern: pattern_uuid=%s", pattern_uuid)
        ...
        self.logger.info("Deleted email pattern: pattern_uuid=%s", pattern_uuid)
    ...
    async def import_patterns_from_csv(
        ...
        """
        self.logger.info("Starting CSV import for email patterns")
        ...
        try:
            ...
            for row_num, row in enumerate(reader, start=2):  # Start at 2 (row 1 is header)
                ...
                try:
                    ...
                    error_details.append(error_msg)
                    errors += 1
                    self.logger.warning("Error processing CSV row %d: %s", row_num, str(e))
                    continue
            ...
            for (company_uuid, pattern_format), pattern_data in pattern_groups.items():
                try:
                    ...
                    error_details.append(error_msg)
                    errors += 1
                    self.logger.warning("Error creating pattern record: %s", str(e))
                    continue
            ...
            self.logger.exception("Failed to import patterns from CSV: %s", str(e))
            raise HTTPException(
    ...
    async def import_patterns_bulk(
        ...
        Returns:
            EmailPatternImportResponse with import statistics
        """
        self.logger.info("Starting bulk import for email patterns: count=%d", len(patterns_data))
        ...
        try:
            ...
            for idx, pattern_data in enumerate(patterns_data, start=1):
                try:
                    ...
                    error_details.append(error_msg)
                    errors += 1
                    self.logger.warning("Error processing pattern %d: %s", idx, str(e))
                    continue
            ...
            await session.rollback()
            self.logger.exception("Failed to import patterns in bulk: %s", str(e))
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
    ...
    async def get_patterns_by_contact(
        ...
        """
        self.logger.debug("Getting patterns for contact: contact_uuid=%s", contact_uuid)
        ...
        # Validate company exists (handle orphaned contacts)
        company = await self.company_repo.get_by_uuid(session, company_uuid)
        if not company:
            self.logger.warning(
                "Orphaned contact detected: contact_uuid=%s has invalid company_id=%s",
                contact_uuid,
                company_uuid,
            )
            # Return response with company_uuid but empty patterns (orphaned contact)
            return {
                "contact_uuid": contact_uuid,
                "company_uuid": company_uuid,  # Keep company_uuid even though company doesn't exist
                "pattern_format": None,
                "pattern_string": None,
                "contact_count": None,
                "is_auto_extracted": None,
        ...
        # Get all patterns for the company
        patterns = await self.email_pattern_repo.get_by_company_uuid(session, company_uuid)
    ...
    async def get_patterns_by_contacts_batch(
        ...
        """
        self.logger.debug("Getting patterns for contacts batch: count=%d", len(contact_uuids))
        
        if not contact_uuids:
            return []
        
        # Batch fetch all contacts in one query
        contacts_stmt = select(Contact).where(Contact.uuid.in_(contact_uuids))
        ...
        for contact_uuid in contact_uuids:
            ...
            # Get patterns for this company (from batch-fetched data)
            patterns = patterns_by_company.get(company_uuid, [])
            
            # Build hash map for O(1) pattern format lookups
            patterns_dict = {p.pattern_format: p for p in patterns if p.pattern_format}
        ...
        self.logger.debug("Batch pattern lookup completed: requested=%d processed=%d", len(contact_uuids), len(results))
        return results
