...
class BulkService:
    ...
    async def bulk_insert(
        ...
        """
        logger.info("Starting bulk insert: records=%d", len(data_list))
        ...
        if company_data_list:
            logger.info("Processing companies: count=%d", len(company_data_list))
        ...
        if contact_data_list:
            logger.info("Processing contacts: count=%d", len(contact_data_list))
        ...
        try:
            ...
            logger.exception("Failed to commit bulk insert transaction")
            raise
    ...
    async def _upsert_companies(
        ...
        for idx, data in enumerate(data_list):
            try:
                ...
                if linkedin_url and linkedin_url != PLACEHOLDER_VALUE:
                    ...
                    if normalized_url:
                        ...
                        if company_match:
                            ...
                            logger.debug("Found existing company by LinkedIn URL: uuid=%s url=%s", company.uuid, normalized_url)
                ...
                error_msg = str(e)
                logger.warning("Error processing company row %d: %s", idx, error_msg)
                errors.append(
                    BulkInsertError(
                        index=idx,
                        error=error_msg,
        ...
        try:
            ...
            logger.exception("Database upsert error for companies")
    ...
    async def _upsert_contacts(
        ...
        for idx, data in enumerate(data_list):
            try:
                ...
                if person_linkedin_url and person_linkedin_url != PLACEHOLDER_VALUE:
                    ...
                    if normalized_url:
                        ...
                        if contact_match:
                            ...
                            logger.debug("Found existing contact by LinkedIn URL: uuid=%s url=%s", contact.uuid, normalized_url)
                ...
                error_msg = str(e)
                logger.warning("Error processing contact row %d: %s", idx, error_msg)
                errors.append(
                    BulkInsertError(
                        index=idx,
                        error=error_msg,
        ...
        try:
            ...
            logger.exception("Database upsert error for contacts")
            for idx in range(len(data_list)):
                errors.append(
                    BulkInsertError(
                        index=idx,
