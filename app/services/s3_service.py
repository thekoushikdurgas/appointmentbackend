...
class S3Service:
    ...
    async def list_csv_files(self, prefix: str = "", bucket_name: Optional[str] = None) -> List[dict]:
        ...
        logger.info("Listing CSV files in S3: bucket=%s prefix=%s", bucket, prefix)
        ...
        try:
            async with session.client("s3") as s3_client:
                ...
                try:
                    ...
                    logger.debug("Bucket '%s' exists and is accessible", bucket)
                ...
                logger.info("Listed %d total object(s), found %d CSV file(s) in bucket '%s'", total_objects, len(files), bucket)
                
                # If no CSV files found but prefix was specified, log warning
                if len(files) == 0 and prefix:
                    logger.warning("No CSV files found with prefix '%s' in bucket '%s' (found %d total objects)", prefix, bucket, total_objects)
                elif len(files) == 0:
                    logger.warning("No CSV files found in bucket '%s' (found %d total objects)", bucket, total_objects)
                
                return files
        except ClientError as exc:
            logger.exception("Failed to list CSV files in S3: error=%s", str(exc))
            raise Exception(f"Failed to list CSV files in S3: {str(exc)}") from exc
        except Exception as exc:
            logger.exception("Unexpected error listing CSV files in S3")
    ...
    async def get_csv_file_info(self, s3_key: str, bucket_name: Optional[str] = None) -> dict:
        ...
        logger.debug("Getting CSV file info from S3: bucket=%s key=%s", bucket, s3_key)
        ...
        try:
            ...
            if error_code == "404" or error_code == "NoSuchKey":
                logger.warning("CSV file not found in S3: key=%s", s3_key)
                raise FileNotFoundError(f"CSV file not found in S3: {s3_key}") from exc
            logger.exception("Failed to get CSV file info from S3: key=%s error=%s", s3_key, str(exc))
            raise Exception(f"Failed to get CSV file info from S3: {str(exc)}") from exc
        except Exception as exc:
            logger.exception("Unexpected error getting CSV file info from S3: key=%s", s3_key)
    ...
    async def read_csv_paginated(
        ...
        logger.debug(
            "Reading paginated CSV from S3: bucket=%s key=%s limit=%d offset=%d",
            bucket,
            s3_key,
            limit,
            offset,
        ...
        try:
            async with session.client("s3") as s3_client:
                ...
                logger.debug("Read %d rows from CSV (offset=%d, limit=%d)", len(rows), offset, limit)
            ...
            if error_code == "404" or error_code == "NoSuchKey":
                logger.warning("CSV file not found in S3: key=%s", s3_key)
                raise FileNotFoundError(f"CSV file not found in S3: {s3_key}") from exc
            logger.exception("Failed to read CSV from S3: key=%s error=%s", s3_key, str(exc))
            raise Exception(f"Failed to read CSV from S3: {str(exc)}") from exc
        except Exception as exc:
            logger.exception("Unexpected error reading CSV from S3: key=%s", s3_key)
    ...
    async def download_csv_file(self, s3_key: str, bucket_name: Optional[str] = None) -> bytes:
        ...
        logger.info("Downloading CSV file from S3: bucket=%s key=%s", bucket, s3_key)

        session = self._get_session()
        try:
            async with session.client("s3") as s3_client:
                logger.debug("Attempting to get object: bucket=%s key=%s", bucket, s3_key)
                response = await s3_client.get_object(Bucket=bucket, Key=s3_key)
                file_content = await response["Body"].read()
                logger.info("CSV file downloaded successfully: bucket=%s key=%s size=%d", bucket, s3_key, len(file_content))
                return file_content
        except ClientError as exc:
            error_code = exc.response.get("Error", {}).get("Code", "")
            if error_code == "404" or error_code == "NoSuchKey":
                logger.warning("CSV file not found in S3: bucket=%s key=%s", bucket, s3_key)
                # Try to list ALL files in the bucket to help debug
                try:
                    logger.info("Listing ALL files in bucket '%s' to help diagnose...", bucket)
                    async with session.client("s3") as s3_client_debug:
                        ...
                        if all_files:
                            logger.info("Found %d total file(s) in bucket '%s':", len(all_files), bucket)
                            # Show all files (up to 50)
                            for file_key in all_files[:50]:
                                logger.info("  - %s", file_key)
                            if len(all_files) > 50:
                                logger.info("  ... and %d more files", len(all_files) - 50)
                            ...
                            if similar_files:
                                logger.info("Similar files found (might be what you're looking for):")
                                for similar in similar_files[:10]:
                                    logger.info("  - %s", similar)
                        else:
                            logger.warning("Bucket '%s' is EMPTY - no files found at all!", bucket)
                except Exception as list_exc:
                    logger.debug("Could not list files for debugging: %s", str(list_exc))
                raise FileNotFoundError(f"CSV file not found in S3 bucket '{bucket}': {s3_key}") from exc
            logger.exception("Failed to download CSV file from S3: key=%s error=%s", s3_key, str(exc))
            raise Exception(f"Failed to download CSV file from S3: {str(exc)}") from exc
        except Exception as exc:
            logger.exception("Unexpected error downloading CSV file from S3: key=%s", s3_key)
