...
class CreditService:
    ...
    def should_deduct_credits(self, user_role: str) -> bool:
        ...
        if user_role in UNLIMITED_CREDITS_ROLES:
            self.logger.debug("User has unlimited credits: role=%s", user_role)
            return False
        return True
    ...
    async def deduct_credits(
        ...
        """
        self.logger.info(
            "Deducting credits: user_id=%s amount=%d",
            user_id,
            amount,
        )
        ...
        if not profile:
            self.logger.warning("Profile not found for credit deduction: user_id=%s", user_id)
        ...
        self.logger.info(
            "Credit deduction: user_id=%s current=%d amount=%d new=%d",
            user_id,
            current_credits,
            amount,
            new_credits,
        ...
        if new_credits < 0:
            self.logger.warning(
                "User credits went negative: user_id=%s balance=%d",
                user_id,
                new_credits,
            )
    ...
    async def get_user_credits(self, session: AsyncSession, user_id: str) -> int:
        ...
        profile = await self.profile_repo.get_by_user_id(session, user_id)
        if not profile:
            self.logger.warning("Profile not found for credit check: user_id=%s", user_id)
            return 0
        
        credits = profile.credits or 0
        self.logger.debug("User credits: user_id=%s credits=%d", user_id, credits)
        return credits
