...
class ExportService:
    ...
    async def generate_company_csv(
        ...
        # Batch fetch all companies and their metadata
        company_uuids_set = set(company_uuids)
        
        # Fetch companies using minimal query (no joins)
        stmt = self.company_repo.base_query_minimal()
        stmt = stmt.where(Company.uuid.in_(company_uuids_set))
        result = await session.execute(stmt)
        companies = {company.uuid: company for company in result.scalars().all()}
        
        # Batch fetch company metadata
        company_metadata = await batch_fetch_company_metadata_by_uuids(session, company_uuids_set)
        ...
        for company_uuid in company_uuids:
                try:
                    company = companies.get(company_uuid)
                    if not company:
                        continue
                    
                    company_meta = company_metadata.get(company_uuid)
