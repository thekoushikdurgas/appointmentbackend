...
class ApolloAnalysisService:
    ...
    def _extract_url_parameters(
        self, url: str, parsed_url
    ) -> tuple[Optional[str], Optional[str], Dict[str, List[str]]]:
        """
        Extract hash path, query string, and parsed parameters from Apollo URL.
        ...
        Returns:
            Tuple of (hash_path, query_string, params_dict)
        """
        ...
        return hash_path, query_string, params

    def _categorize_parameters(
        self, params: Dict[str, List[str]]
    ) -> tuple[Dict[str, Dict[str, ParameterDetail]], int]:
        """
        Categorize Apollo parameters into their respective categories.
        
        Args:
            params: Dictionary of parameter names to lists of values
            
        Returns:
            Tuple of (categorized_params_dict, total_parameter_values_count)
        """
        ...
        for param_name, param_values in params.items():
            ...
            category_name = self._get_parameter_category(param_name)
            description = self.PARAM_DESCRIPTIONS.get(param_name, "Filter parameter")
        ...
        return categorized_params, total_parameter_values

    def _get_parameter_category(self, param_name: str) -> str:
        """Get the category name for a parameter, defaulting to 'Other'."""
    ...
    def _build_categories_list(
        self, categorized_params: Dict[str, Dict[str, ParameterDetail]]
    ) -> List[ParameterCategory]:
        """Build the list of ParameterCategory objects from categorized parameters."""
    ...
    async def analyze_url(self, url: str) -> ApolloUrlAnalysisResponse:
        ...
        if self.cache.enabled:
            try:
                ...
                if cached_result:
                    # Cache hit - reconstruct response from cached data
                    return ApolloUrlAnalysisResponse(**cached_result)
            except Exception:
                # Error reading from cache - proceed with analysis (non-critical)
                pass
        ...
        try:
            ...
            hash_path, query_string, params = self._extract_url_parameters(url, parsed_url)
            ...
            # Categorize parameters and build response structure
            categorized_params, total_parameter_values = self._categorize_parameters(params)
            categories_list = self._build_categories_list(categorized_params)
            ...
            if self.cache.enabled:
                try:
                    ...
                    # Error caching URL analysis result (non-critical)
                    pass
    ...
    def _process_person_titles(
        self,
        titles: list[str],
        include_similar: bool,
        contact_filters: dict,
    ) -> None:
        """Process personTitles[] parameter based on includeSimilarTitles flag.
        
        Args:
            titles: List of title strings from Apollo
            include_similar: Whether to use jumble mapping (True) or exact mapping (False)
            contact_filters: Dictionary to update with title filters
        """
        if include_similar:
    ...
    def map_to_contact_filters(
        ...
        if "sortByField" in raw_parameters:
            ...
            if sort_field and sort_field.lower() not in ["[none]", "none", ""]:
                ...
                if mapped_field is None:
                    # Unknown Apollo sort field - use default
        ...
        if "personTitles[]" in raw_parameters:
            ...
            if titles:
                self._process_person_titles(titles, include_similar, contact_filters)
            ...
            for range_str in ranges:
                ...
                if len(parts) == 2:
                    try:
                        ...
                        # Invalid employee range format - skip this range
                        pass
            if all_mins:
            ...
            if apollo_param in raw_parameters:
                ...
                if tag_ids:
                    ...
                    if industry_names:
                        ...
                        mapped_params.add(apollo_param)
                    # else: No valid industry names found - skip this parameter
