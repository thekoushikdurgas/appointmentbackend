...
class ConnectionManager:
    ...
    async def connect(self, websocket: WebSocket, chat_id: str) -> None:
        ...
        logger.info(
            "WebSocket connected: chat_id=%s total_connections=%d",
            chat_id,
    ...
    def disconnect(self, websocket: WebSocket) -> None:
        ...
        if chat_id and chat_id in self.active_connections:
            ...
            logger.info(
                "WebSocket disconnected: chat_id=%s remaining_connections=%d",
                chat_id,
                len(self.active_connections.get(chat_id, set())),
            )
    ...
    async def send_to_chat(self, message: dict, chat_id: str) -> None:
        ...
        """
        if chat_id not in self.active_connections:
            logger.debug("No connections for chat_id=%s", chat_id)
            return
