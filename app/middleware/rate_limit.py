...
class RateLimiter:
    ...
    async def check_rate_limit(self, client_id: str) -> None:
        ...
        if len(self.clients[client_id]) >= self.requests:
            logger.warning(
                "Rate limit exceeded: client_id=%s requests=%d/%d window=%ds",
                client_id,
                len(self.clients[client_id]),
                self.requests,
                self.window,
            )
            raise HTTPException(
        ...
        logger.debug(
            "Rate limit check passed: client_id=%s requests=%d/%d",
            client_id,
            len(self.clients[client_id]),
            self.requests,
        )
...
def get_ai_rate_limiter() -> RateLimiter:
    ...
    if _ai_rate_limiter is None:
        ...
        logger.info(
            "AI rate limiter initialized: requests=%d window=%ds",
            ai_settings.AI_RATE_LIMIT_REQUESTS,
            ai_settings.AI_RATE_LIMIT_WINDOW,
        )
